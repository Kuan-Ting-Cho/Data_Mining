...
df.loc[df[(df['reward'] >= 2) & (df['reward'] <= 6) &
          (df['time_cost'] <= -6)].index, 'answer'] = 1
df.loc[df[(df['reward'] >= 2) & (df['reward'] <= 6)
          & (df['feel'] >= 6)].index, 'answer'] = 1
df.loc[df[(df['reward'] >= 2) & (df['reward'] <= 6) &
          (df['ability'] >= 6)].index, 'answer'] = 1
df.loc[df[(df['reward'] >= 2) & (df['reward'] <= 6) & (0 <= df['time_cost']) & (df['time_cost'] <= 4) & (
    2 <= df['ability']) & (df['ability'] <= 6) & (2 <= df['feel']) & (df['feel'] <= 6)].index, 'answer'] = 1
df.loc[df[(df['reward'] >= 2) & (df['reward'] <= 6) & (df['time_cost'] >= 4) & (2 <= df['feel']) & (
    df['feel'] <= 6) & (-6 <= df['ability']) & (df['ability'] <= -2)].index, 'answer'] = 1


print('Rule1 numbers: ', len(
    df[(df['reward'] >= 2) & (df['reward'] <= 6) & (df['time_cost'] <= -6)].index))
print('Rule2 numbers: ', len(
    df[(df['reward'] >= 2) & (df['reward'] <= 6) & (df['feel'] >= 6)].index))
print('Rule3 numbers: ', len(
    df[(df['reward'] >= 2) & (df['reward'] <= 6) & (df['ability'] >= 6)].index))
print('Rule4 numbers: ', len(df[(df['reward'] >= 2) & (df['reward'] <= 6) & (0 <= df['time_cost']) & (df['time_cost'] <= 4) & (
    2 <= df['ability']) & (df['ability'] <= 6) & (2 <= df['feel']) & (df['feel'] <= 6)].index))
print('Rule5 numbers: ', len(df[(df['reward'] >= 2) & (df['reward'] <= 6) & (df['time_cost'] >= 4) & (2 <= df['feel']) & (
...
df.to_csv('./input/data4_with_words.csv', index=False)
df_values.to_csv('./input/data4.csv', index=False)
